...
namespace simulation {
    ...
    struct Clock {
        ...
        std::shared_ptr<TraceType> trace;
        bool level = false;
        ...
        Clock(const std::string &n, uint64_t ps, std::shared_ptr<DutType> dut, std::shared_ptr<TraceType>trace) : name(n), period_ps(ps), dut(dut), trace(trace) {}

...
simulation::Task<> DualPortRamBaseSequence::write(uint32_t addr, uint32_t data) {
    this->log_debug(std::format("Issuing write: addr=0x{:X}, data=0x{:X}", addr, data));
    auto txn = dispatch_write(addr, data);
    this->log_debug_txn(txn->txn_id, "Waiting for write completion");
    co_await wait_for_txn_done(txn);
    this->log_debug_txn(txn->txn_id, "Write completed");
}

simulation::Task<> DualPortRamBaseSequence::read(uint32_t addr) {
    this->log_debug(std::format("Issuing read: addr=0x{:X}", addr));
    auto txn = dispatch_read(addr);
    this->log_debug_txn(txn->txn_id, "Waiting for read completion");
    co_await wait_for_txn_done(txn);
}

simulation::Task<> DualPortRamBaseSequence::wait_wr_cycles(uint32_t n) {
    if (!p_sequencer) {
        this->log_error("Sequencer not connected. Cannot wait for clock.");
        co_return;
    }

    this->log_debug(std::format("Waiting for {} write clock cycles", n));
    ...
    this->log_debug(std::format("Waited {} write clock cycles", n));
}
...
simulation::Task<> DualPortRamBaseSequence::wait_rd_cycles(uint32_t n) {
    ...
    this->log_debug(std::format("Waiting for {} read clock cycles", n));
    ...
    this->log_debug(std::format("Waited {} read clock cycles", n));
}

...
simulation::Task DualPortRamTopSequence::body() {
    ...
    auto directed_all_address_increment_wr_rd_seq = std::make_unique<Seq_Directed_WriteRead_All_Address_Increment>(wr_addr_width_, wr_data_width_);
    ...
    directed_all_address_increment_wr_rd_seq = std::make_unique<Seq_Directed_WriteRead_All_Address_Increment>(wr_addr_width_, wr_data_width_);
    co_await p_sequencer->start_sequence(std::move(directed_all_address_increment_wr_rd_seq));
    std::cout << "---------------------------------------\n";

    co_await wait_wr_cycles(5U);

    auto directed_all_address_decrement_wr_rd_seq = std::make_unique<Seq_Directed_WriteRead_All_Address_Decrement>(wr_addr_width_, wr_data_width_);

...
public:
    ...
    void add_expected_transaction(TransactionPtr transaction, uint64_t expected_cycle) {
        ...
        expected_transactions_.emplace(transaction, expected_cycle, current_cycle);
     ...
     */
    virtual std::uint32_t check_current_cycle(Transaction &actual) {
        ...
        while (!expected_transactions_.empty() && 
            ...
            auto& expected = expected_transactions_.front();

            // Check for timeout
            if (config_.enable_timeout_checking &&
                (current_cycle - expected.submitted_cycle) > config_.timeout_cycles) {
                handle_timeout(expected);
            } else {
                // Perform the actual check
                bool check_result = checker_.perform_check(expected, actual);
...
protected:
    ...
    virtual void handle_timeout(const ExpectedTransaction& expected) {
        std::uint64_t current_cycle = ctx_->current_cycle();
        log_error("Transaction timed out: " + expected.transaction->convert2string() +

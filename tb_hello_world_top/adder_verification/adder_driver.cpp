...
AdderDriver::AdderDriver(const std::string& name, DutPtr dut)
    : Base(name, dut), config_(Config{}) {
...
AdderDriver::AdderDriver(const std::string& name, DutPtr dut, const Config& config)
    : Base(name, dut), config_(config) {
    current_inputs_ = {0, 0, 0, false};
    reset();
    log_info("AdderDriver initialized with pipeline depth " + std::to_string(config_.pipeline_depth));
...
void AdderDriver::drive_transaction(const AdderTransaction& txn, uint64_t cycle) {
    uint8_t a = txn.get_a();
    uint8_t b = txn.get_b();
    ...
    // Update tracking
    update_current_inputs(a, b, cycle);
...
void AdderDriver::pre_drive(const AdderTransaction& txn, uint64_t cycle) {
...
void AdderDriver::update_current_inputs(uint8_t a, uint8_t b, uint64_t cycle) {
    current_inputs_.a = a;
    current_inputs_.b = b;
    current_inputs_.cycle = cycle;
